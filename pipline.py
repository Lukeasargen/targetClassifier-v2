...
from util import quantize_to_int, pil_loader, get_device
...
def detect_pipline(model, pil_image, thresh=0.5, expand_ratio=1, min_size=0):
    ...
    img_ten = T.ToTensor()(cropped).unsqueeze(0).to(get_device())
    with torch.no_grad():
        preds = model(img_ten).cpu()
...
def classify_pipline(model, patch, img_size=32):
    ...
    img_ten = T.ToTensor()(resized).unsqueeze(0).to(get_device())
    with torch.no_grad():
        preds = model(img_ten)
...
def main():
    print(get_device())
    classify_model = torch.jit.load("dev/classify.pt", map_location=get_device())
    segment_model = torch.jit.load("dev/detect.pt", map_location=get_device())
    root = r'C:\Users\lukeasargen\projects\aerial_real/'  # image folder, aerial_backgrounds, aerial_cropped, aerial_real
